...
module Readability
  class Document
    ...
    WHITELIST_TAGS       = %w[div p img a strong small em]
    BLACKLIST_ATTRIBUTES = %w[class style id]

    REGEXES = {
        :unlikelyCandidatesRe => /combx|comment|disqus|foot|header|menu|meta|nav|rss|shoutbox|sidebar|sponsor/i,
        :okMaybeItsACandidateRe => /and|article|body|column|main/i,
        :positiveRe => /main|article|body|image|content|entry|hentry|page|pagination|post|text/i,
...
#          end
    ...
    def sanitize(node, candidates, options = {})
      ...
      base_whitelist = @options[:tags] || WHITELIST_TAGS
      ...
      ([node] + node.css("*")).each do |el|
        ...
        if whitelist[el.node_name]
          ...
          el.attributes.each { |a, x| el.delete(a) if BLACKLIST_ATTRIBUTES.include?(a.to_s) }
      ...
      # Get rid of duplicate whitespace && comments
      node.to_html.gsub(/[\r\n\f]+/, "\n" ).gsub(/[\t ]+/, " ").gsub(/&nbsp;/, " ").gsub(/\<![ \r\n\t]*(--([^\-]|[\r\n]|-[^\-])*--[ \r\n\t]*)\>/, '')
    end
